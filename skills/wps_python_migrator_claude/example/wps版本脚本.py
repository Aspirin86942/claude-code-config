...
def normalize_error_log(error_log: pd.DataFrame, run_id: str) -> pd.DataFrame:
    ...
    normalized_df["error_code"] = normalized_df["error_code"].map(normalize_error_code)
...
def normalize_display_sec_code_series(
    ...
    """
    code_series = (
        stock_code_series.astype("string")
        .str.strip()
        .str.replace(r"\.0$", "", regex=True)
    )
...
def build_stock_route_frame(
    ...
    if hk_a_share_mask.any():
        ...
        if missing_digits_mask.any():
            invalid_org_id = (
                org_series.loc[hk_a_share_mask].loc[missing_digits_mask].iloc[0]
            )
    ...
    if pure_hk_mask.any():
        hk_fetch_code = "HK" + display_code_series.loc[pure_hk_mask].str.lstrip(
            "0"
        ).str.zfill(4)
...
def extract_concepts_from_html(html_text: str) -> tuple[list[str], str]:
    ...
    raw_concept_text = (
        ...
        else dd_tag.get_text(strip=True)
    )
...
def build_run_summary(
    ...
    blocking_error_count = (
        0
        if error_log.empty
        else int(error_log["severity"].astype("string").str.upper().eq("ERROR").sum())
    )
